...
class Experiment:
    ...
    def train_fold(self, fold):
        ...
        logger.info("working dir is %s", fold_dir)
        ...
        logger.info(
            "  Data Set %s: %s  train cases, %s vald cases",
            fold,
            train_files.size,
            vald_files.size,
        )
    ...
    def apply_fold(self, fold: int):
        ...
        tqdm.write(
            f"Finished applying {self.name} {folder_name} (Fold {fold+1} of {self.folds})"
        )
    ...
    def evaluate_fold(self, fold: int):
        ...
        for version in self.versions:
            ...
            if eval_files_exist:
                ...
                self.evaluate_fold_version(folder_name, test_files, version=version)
        ...
        tqdm.write(
            f"Finished evaluating {self.name} {folder_name} (Fold {fold+1} of {self.folds})"
        )

    def run_fold(self, fold: int):
        """Run a complete fold with training, application and evaluation
